...
function SettingsPage() {
    ...
    // Profile form state - Added profileImageUrl
    const [profile, setProfile] = useState({ firstName: '', lastName: '', birthDate: '', profileImageUrl: '' });
    ...
    useEffect(() => {
        if (userPerson) {
            setProfile({
                ...
                profileImageUrl: userPerson.profileImageUrl || '' // Load existing image URL
            });
        }
    }, [userPerson]);
    ...
    const saveProfile = async () => {
        ...
        try {
            // The 'profile' state now includes profileImageUrl, so it gets saved too
    ...
    // 3. ADD THE UPLOAD HANDLER FUNCTION
    const handleProfileImageUpload = async (e) => {
        const file = e.target.files[0];
        if (!file) return;
        setProfileStatus(''); // Clear any previous status messages

        const imageUrl = await upload(file); // This calls the hook
        if (imageUrl) {
            // Update state to show the new image immediately
            setProfile(prev => ({ ...prev, profileImageUrl: imageUrl }));
            
            // Save the new image URL to Firestore immediately for better UX
            try {
                await updateUserProfile({ personId: userPerson.id, profileImageUrl: imageUrl });
                setProfileStatus('Picture updated');
            } catch (err) {
                console.error("Failed to save new profile picture:", err);
                setProfileStatus('Picture save failed');
            }
        } else {
            // The hook sets its own error state, but you can set one here too
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-10 p-4">
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    <div className="space-y-4 md:col-span-2">
                        ...
                        <div className="pt-2 flex gap-3 items-center">
                            <button onClick={saveProfile} disabled={savingProfile || uploading} className="btn btn-primary disabled:opacity-50">{savingProfile? 'Saving...' : 'Save Profile'}</button>
                        </div>
                    </div>
                    {/* 4. UPDATE THE JSX FOR IMAGE UPLOAD */}
                    <div className="flex flex-col md:col-span-1 items-center justify-center">
                        <label htmlFor="profile-upload" className="cursor-pointer group relative">
                            <div className="h-32 w-32 rounded-full bg-accent/20 flex items-center justify-center text-4xl font-bold text-accent select-none overflow-hidden">
            ...
            {/* Appearance Section (No changes here) */}
            <section className="card p-6 space-y-6">
                <div className="flex items-center justify-between">
            ...
            {/* Account Section (No changes here) */}
...
// Accent circle inline style (No changes here)
